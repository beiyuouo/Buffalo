...
from evalcap.acc.acc import Acc
from evalcap.acc.ynacc import YNAcc
from evalcap.acc.osacc import OSAcc
...
class MVQAModel(pl.LightningModule):
    """
    R2GenGPT model.
    """
    ...
    def __init__(self, cfg):
        ...
        # self.tokenizer.add_special_tokens({"img_token": "<image>"})
        ...
        self.prompt = "You are an AI assistant specialized in biomedical topics. You are designed to assist human with a variety of medical and clinical research tasks using natural language.  You are able to understand the visual content that the user provides, and assist the user with a variety of medical and clinical research tasks using natural language. You are able to understand the visual content that the user provides, and assist the user with a variety of medical and clinical research tasks using natural language. Your task is to generate an answer between a person (Human) inquiring about the image and you (Assistant) responding to their questions breifly. The conversation should proceed as though both the Human and Assistant are viewing the image. Answer the following medical question in a simple word; explaining and reasoning is not needed. Say yes or no for a yes/no question; for other questions, just give the answer directly. Follow the instructions carefully."
    ...
    def score(self, ref, hypo):
        ...
        """
        scorers = [(Acc(), "Acc"), (YNAcc(), "YNAcc"), (OSAcc(), "OSAcc"), (Bleu(4), ["Bleu_1", "Bleu_2", "Bleu_3", "Bleu_4"]), (Rouge(), "ROUGE_L"), (Meteor(), "METEOR"), (Cider(), "CIDEr")]
        final_scores = {}
    ...
    def prompt_wrap(self, img_embs, img_atts, question=""):
        prompt = f"{self.prompt} Human: <Img><ImageHere></Img> Here is the question: {question} Assistant:"
        ...
        p_after_tokens = self.tokenizer(p_after, return_tensors="pt", add_special_tokens=False).to(
            img_embs.device
        )
    ...
    def forward(self, samples):
        ...
        if self.cfg.train.modal_hetero and mm_str == "mean" and mm_mean_type == "original":
            # logger.debug(f"Use strategy {mm_str}, mean_type {mm_mean_type}, processing images")
            means = self.cfg.payload["means"]
            if client_type == 1:
                image = [torch.from_numpy(means[0]).expand(image[0].shape[0], -1, -1, -1).to(image[0].device)]

        if self.cfg.train.modal_hetero and mm_str == "mean" and mm_mean_type == "token":
            # logger.debug(f"Use strategy {mm_str}, mean_type {mm_mean_type}, processing images")
            means = self.cfg.payload["means"]
            if client_type == 1:
                missing_embs = torch.from_numpy(means[0]).expand(img_embs.shape[0], -1, -1).to(image[0].device)
                img_embs = self.proj(missing_embs)
            else:
                img_embs, img_atts, img_feats = self.encode_img(image, return_feats=True)
        elif self.cfg.train.modal_hetero and mm_str == "prototype" and (client_type == 1):
            ...
            if global_prototype is None:
                ...
                def get_txt_emb():
                    ...
                    text = [t + self.eos_token for t in samples["answer"]]
                ...
                _, img_seq_len, img_embed_dim = (batch_size, 49, img_protos[0][0].shape[-1])
                ...
                for bs in range(batch_size):
                    ...
                    for idx, img_proto_id in enumerate(miss_emb_ids):
                        ...
                        img_proto_ = torch.tensor(img_proto[0]).to(image[0].device) + torch.normal(0, img_proto[1], img_proto[0].shape).to(
                            image[0].device
                        )
                ...
                miss_embs = miss_embs.reshape(batch_size, img_seq_len, -1).to(image[0].device)
                img_feats = miss_embs
                img_embs = self.proj(torch.stack([miss_embs]).mean(0))
                img_atts = torch.ones(img_embs.size()[:-1], dtype=torch.long).to(image[0].device)
        ...
        if self.cfg.train.modal_hetero and mm_str == "mean" and mm_mean_type == "original" and client_type == 2:
            # logger.debug(f"Use strategy {mm_str}, mean_type {mm_mean_type}, client type {client_type}, processing text")
            means = self.cfg.payload["means"]

            to_regress_tokens = torch.from_numpy(means[1]).unsqueeze(0).expand(img_embs.shape[0], -1).to(img_embs.device)
            targets = to_regress_tokens.masked_fill(to_regress_tokens == 0, -100)

            empty_targets = (
                torch.ones([img_atts.shape[0], img_atts.shape[1] + 1], dtype=torch.long).to(image[0].device).fill_(-100)
            )  # plus one for bos
            targets = torch.cat([empty_targets, targets], dim=1)

            batch_size = img_embs.shape[0]
            bos = torch.ones([batch_size, 1], dtype=to_regress_tokens.dtype, device=to_regress_tokens.device) * self.tokenizer.bos_token_id
            bos_embeds = self.embed_tokens(bos)
            bos_atts = img_atts[:, :1]
        ...
        elif self.cfg.train.modal_hetero and mm_str == "mean" and mm_mean_type == "token" and client_type == 2:
            # logger.debug(f"Use strategy {mm_str}, mean_type {mm_mean_type}, client type {client_type}, processing text")
            means = self.cfg.payload["means"]
            to_regress_embeds = torch.from_numpy(means[1]).unsqueeze(0).expand(img_embs.shape[0], -1, -1).to(img_embs.device)

            # logger.debug(f"to_regress_embeds.shape: {to_regress_embeds.shape}")
        elif self.cfg.train.modal_hetero and mm_str == "prototype" and client_type == 2:
            # logger.debug(f"Use strategy {mm_str}, computing prototype loss")
            global_prototype = getattr(self.cfg.payload, "global_prototype", None)
            # TODO
            if global_prototype is None:
                ...
                for bs in range(batch_size):
                    ...
                    for txt_proto_id in miss_emb_ids:
                        ...
                        txt_proto_ = torch.tensor(txt_proto[0]).to(img_feats.device) + torch.normal(0, txt_proto[1], txt_proto[0].shape).to(
                            img_feats.device
                        )
    ...
    def save_checkpoint(self, eval_res):
        ...
        if getattr(self.cfg, "client_idx", None) is not None:
            ...
            save_to = os.path.join(
                self.cfg.save_dir,
                "checkpoints",
                "checkpoint_client{}_round{}_epoch{}_step{}_bleu{:3f}_cider{:3f}.pth".format(
                    client_idx, round_idx, current_epoch, global_step, eval_res["Bleu_4"], eval_res["CIDEr"]
                ),
            )
    ...
    def validation_step(self, samples, batch_idx):
        ...
        img_embs, img_atts = self.prompt_wrap(img_embs, img_atts, question)
        ...
        if self.stage == None or self.stage == 1:
            optimizer = torch.optim.AdamW(
                [
                    ...
                    {"params": self.proj.parameters(), "lr": self.cfg.train.lr},
                    {"params": self.layer_norm.parameters(), "lr": self.cfg.train.lr},
                    {"params": self.model.parameters(), "lr": self.cfg.train.lr_ed},
                ]
            )
    ...
    def get_prototype(self, dm):
        ...
        with torch.no_grad():
            for samples in dm.train_dataloader():
                ...
                _, _, img_embs = self.encode_img([image[0]], return_feats=True)
                text = [t + self.eos_token for t in samples["answer"]]
                ...
                cross_proto = [img_embs.cpu().numpy(), text_embs.cpu().numpy()]
        ...
        for cross_proto in cross_protos:
            img_proto = cross_proto[0]  # 4,49,1024 + 4,49,1024 = 4,49,1024
            text_proto = cross_proto[1]  # 4,60,4096
        ...
        for i in range(self.cfg.train.cross_fedproto.k):
            ...
            proto_std_i[i] = 0 if np.isnan(proto_std_i[i]) else proto_std_i[i]
            proto_std_t[i] = np.std(text_protos[text_proto_labels == i])
            proto_std_t[i] = 0 if np.isnan(proto_std_t[i]) else proto_std_t[i]
